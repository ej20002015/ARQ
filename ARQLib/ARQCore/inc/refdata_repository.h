...
public:
    ...
    std::shared_ptr<Cache<T>> get() const
    {
        ...
        {
            if( !m_Currencies.load() )
                const_cast<Repository*>(this)->load<Currency>();
            return m_Currencies;
        }
        if constexpr( std::is_same_v<T, User> )
        {
            if( !m_Users.load() )
                const_cast<Repository*>(this)->load<User>();
            return m_Users;
        }
    }
    ...
    void load()
    {
        ...
        if constexpr( std::is_same_v<T, Currency> ) m_Currencies.store( newCache );
        if constexpr( std::is_same_v<T, User> ) m_Users.store( newCache );
...
private:
    ...
    std::atomic<std::shared_ptr<Cache<Currency>>> m_Currencies;
    std::atomic<std::shared_ptr<Cache<User>>> m_Users;
};
