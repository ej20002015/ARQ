 ...
 * Declaration
 */
namespace ARQ { // Surround with ARQ namespace
...
inline ssize_t write_response_line(Stream &strm, int status) {
  ...
  s += ' ';
  s += ARQ::http::status_message(status);
  s += "\r\n";
  return strm.write(s.data(), s.size());
}
...
inline bool SSLClient::check_host_name(const char *pattern,
  ...
  for (const auto &h : host_components_) {
    auto &p = *itr;
    if (!ARQ::http::detail::case_ignore::equal(p, h) && p != "*") {
          ...
          partial_match =
              std::equal(p.begin(), p.begin() + prefix_length, h.begin(),
                         [](const char ca, const char cb) {
                           return ARQ::http::detail::case_ignore::to_lower(ca) ==
                                  ARQ::http::detail::case_ignore::to_lower(cb);
                         });
        }
      }
