...
TEST( DateTest, InvalidDateConstruction )
{
    ...
    EXPECT_THROW( Date( Year( 2024 ), Month::Feb, Day( 30 ) ), ARQException );
    EXPECT_THROW( Date( Year( 2023 ), Month::Feb, Day( 29 ) ), ARQException ); // 2023 is not a leap year
    EXPECT_THROW( Date( Year( 2024 ), Month::Apr, Day( 31 ) ), ARQException ); // April has 30 days
}
...
TEST( DateTest, LeapYearHandling )
{
    ...
    EXPECT_THROW( Date( Year( 2023 ), Month::Feb, Day( 29 ) ), ARQException );

    // Test century years
    Date temp( Year( 1900 ), Month::Feb, Day( 28 ) );
    auto serial = temp.serial();

    EXPECT_THROW( Date( Year( 1900 ), Month::Feb, Day( 29 ) ), ARQException ); // 1900 is not a leap year
