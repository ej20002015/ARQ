...
public: // IStreamProducer implementation
	...
	ARQKafka_API void flush( const std::chrono::milliseconds timeout = 60s )                                                                              override;

	// Transactional API (EOS)
	ARQKafka_API void initTransactions( const std::chrono::milliseconds timeout = 60s )                                                                                                           override;
	ARQKafka_API void beginTransaction()                                                                                                                                                          override;
	ARQKafka_API void commitTransaction( const std::chrono::milliseconds timeout = 60s )                                                                                                          override;
	ARQKafka_API void abortTransaction( const std::chrono::milliseconds timeout = 60s )                                                                                                           override;
	ARQKafka_API void sendOffsetsToTransaction( const std::map<StreamTopicPartition, int64_t>& offsets, const StreamGroupMetadata& groupMetadata, const std::chrono::milliseconds timeout = 60s ) override;
...
public: // IStreamConsumer implementation
	...
	ARQKafka_API std::unique_ptr<IStreamConsumerMessageBatch> poll( const std::chrono::milliseconds timeout, const StreamConsumerReadHeaders readHeaders = StreamConsumerReadHeaders::DEFAULT )              override;
	...
	ARQKafka_API void                                    seek( const StreamTopicPartition& partition, int64_t offset, const std::chrono::milliseconds timeout = 60s )        override;
	ARQKafka_API void                                    seekToBeginning( const std::chrono::milliseconds timeout = 60s )                                                    override;
	ARQKafka_API void                                    seekToBeginning( const std::set<StreamTopicPartition>& partitions, const std::chrono::milliseconds timeout = 60s )  override;
	ARQKafka_API void                                    seekToEnd( const std::chrono::milliseconds timeout = 60s )                                                          override;
	ARQKafka_API void                                    seekToEnd( const std::set<StreamTopicPartition>& partitions, const std::chrono::milliseconds timeout = 60s )        override;
	ARQKafka_API int64_t                                 position( const StreamTopicPartition& partition )                                                                   override;
	ARQKafka_API std::map<StreamTopicPartition, int64_t> beginningOffsets( const std::set<StreamTopicPartition>& partitions, const std::chrono::milliseconds timeout = 60s ) override;
	ARQKafka_API std::map<StreamTopicPartition, int64_t> endOffsets( const std::set<StreamTopicPartition>& partitions, const std::chrono::milliseconds timeout = 60s )       override;
