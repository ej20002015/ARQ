...
spdlog::level::level_enum TMQLevelToSpdlog( const LogLevel level )
{
	switch( level )
	{
	...
	: m_cfg( cfg )
{
	try
	{
		...
		if( !m_cfg.disableConsoleLogger )
		{
			...
			consoleSink->set_level( TMQLevelToSpdlog( m_cfg.consoleLoggerLevel ) );
		...
		if( !m_cfg.disableFileLogger )
		{
			...
			fileSink->set_level( TMQLevelToSpdlog( m_cfg.fileLoggerLevel ) );
		...
		m_logger->flush_on( TMQLevelToSpdlog( m_cfg.flushLevel ) );
...
{
	return m_logger->should_log( TMQLevelToSpdlog( level ) );
}

void Logger::setLevel( const LogLevel level )
{
	m_logger->set_level( TMQLevelToSpdlog( level ) );
}
...
void Logger::logInternal( const LogLevel level, const std::source_location& loc, const Module module, const JSON& contextArgs, std::string&& msg, const std::optional<std::reference_wrapper<const TMQException>> exception )
{
	try
	{
		// Work out context
		...
		// Dispatch to spdlog
		m_logger->log( spdlog::source_loc{}, TMQLevelToSpdlog( level ), logEntry.dump() );
	}
...
Log::Context::Global::Scoped::Scoped( const JSON& contextArgs )
{
	if( !contextArgs.is_object() )
		throw TMQException( "Attempting to create Log::Context::Global::Scoped with non-object JSON" );
...
{
	if( !contextArgs.is_object() )
		throw TMQException( "Attempting to create Log::Context::Thread::Scoped with non-object JSON" );
