...
#ifdef SWIGCSHARP
%rename(sub) TMQ::Time::Date::operator-;
%rename(eq) TMQ::Time::Date::operator==;
#endif

#ifdef SWIGPYTHON
%rename(__sub__) TMQ::Time::Date::operator-;
%rename(__eq__) TMQ::Time::Date::operator==;
#endif
...
public:
    // Constructors
    Date(const Year y, const Month m, const Day d) throw(TMQ::TMQException);
    Date(const int32_t serial) throw(TMQ::TMQException);
