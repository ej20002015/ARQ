...
public:
	...
	[[nodiscard]] TMQUtils_API Date addYears( const int32_t years )   const noexcept;
	[[nodiscard]] TMQUtils_API Date subYears( const int32_t years )   const noexcept;
	[[nodiscard]] TMQUtils_API Date addMonths( const int32_t months ) const noexcept;
	[[nodiscard]] TMQUtils_API Date subMonths( const int32_t months ) const noexcept;
	[[nodiscard]] TMQUtils_API Date addDays( const int32_t days )     const noexcept;
	[[nodiscard]] TMQUtils_API Date subDays( const int32_t days )     const noexcept;
...
{
public:
	TMQUtils_API explicit DateTime( const Date& dt );
	TMQUtils_API explicit DateTime( const Date& dt, const Hour h, const Minute m, const Second s );
	TMQUtils_API explicit DateTime( const Date& dt, const TimeOfDay& tod );
	TMQUtils_API explicit DateTime( const Microseconds us );
	TMQUtils_API explicit DateTime( const std::chrono::system_clock::time_point tp );
	DateTime() = default;
	...
	[[nodiscard]] TMQUtils_API Date         date()             const noexcept;
	[[nodiscard]] TMQUtils_API TimeOfDay    timeOfDay()        const noexcept;
	[[nodiscard]] TMQUtils_API Hour         hour()             const noexcept;
	[[nodiscard]] TMQUtils_API Minute       minute()           const noexcept;
	[[nodiscard]] TMQUtils_API Second       second()           const noexcept;
	[[nodiscard]] TMQUtils_API Millisecond  millisecond()      const noexcept;
	[[nodiscard]] TMQUtils_API Microsecond  microsecond()      const noexcept;
	[[nodiscard]] TMQUtils_API Microseconds microsecondsPast() const noexcept;

	[[nodiscard]] TMQUtils_API Microseconds microsecondsSinceEpoch() const noexcept;

	[[nodiscard]] TMQUtils_API std::chrono::system_clock::time_point tp() const noexcept;

	[[nodiscard]] TMQUtils_API DateTime addYears( const int32_t years )               const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subYears( const int32_t years )               const noexcept;
	[[nodiscard]] TMQUtils_API DateTime addMonths( const int32_t months )             const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subMonths( const int32_t months )             const noexcept;
	[[nodiscard]] TMQUtils_API DateTime addDays( const int32_t days )                 const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subDays( const int32_t days )                 const noexcept;
	[[nodiscard]] TMQUtils_API DateTime addHours( const int64_t hours )               const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subHours( const int64_t hours )               const noexcept;
	[[nodiscard]] TMQUtils_API DateTime addMinutes( const int64_t minutes )           const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subMinutes( const int64_t minutes )           const noexcept;
	[[nodiscard]] TMQUtils_API DateTime addSeconds( const int64_t seconds )           const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subSeconds( const int64_t seconds )           const noexcept;
	[[nodiscard]] TMQUtils_API DateTime addMilliseconds( const int64_t milliseconds ) const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subMilliseconds( const int64_t milliseconds ) const noexcept;
	[[nodiscard]] TMQUtils_API DateTime addMicroseconds( const int64_t microseconds ) const noexcept;
	[[nodiscard]] TMQUtils_API DateTime subMicroseconds( const int64_t microseconds ) const noexcept;
	...
	TMQUtils_API friend std::ostream& operator<<( std::ostream& os, const DateTime& date );
...
public:
	TMQUtils_API static const DateTime MIN;
	TMQUtils_API static const DateTime MAX;
...
template <typename CharT>
struct formatter<TMQ::Time::DateTime, CharT> : formatter<std::chrono::system_clock::time_point, CharT>
{
	template <typename FormatContext>
	auto format( const TMQ::Time::DateTime& dateTime, FormatContext& ctx ) const -> decltype( ctx.out() )
	{
		if( dateTime.isSet() )
			return formatter<std::chrono::system_clock::time_point, CharT>::format( dateTime.m_tp.value(), ctx );
		else
			return std::format_to( ctx.out(), "{}", TMQ::Time::DateTime::NOT_SET_STR );
	}
};
